...
const createCsurfToken = () => (request, response, next) => {
...
module.exports = {
  register: ({ app, excludedPaths }) => {
    const excludedPathsRegex = (excludedPaths || []).map(path =>
      PathToRegex(path.startsWith('/') ? path : `/${path}`)
    )
    ...
    app.use((request, response, next) => {
      ...
      const queryAccepted =
        query === '{csrf_token}' ||
        (query?.startsWith('query') && query?.endsWith('{csrf_token}'))

      const pathIsExcluded = excludedPathsRegex.find(
        regex => regex.test(request.path) === true
      )
      if (
        (request.method === 'POST' &&
          typeof query === 'string' &&
