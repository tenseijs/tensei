...
const CreateResource: React.FC<CreateResourceProps> = ({}) => {
  ...
  if (
    ...
    (!params.id && !window.Tensei.state.permissions[`insert:${resource.slug}`])
  ) {
  ...
  const onSave = () => {
    ...
    window.Tensei.client[isEditing ? 'patch' : 'post'](
      ...
      .then(() => {
        ...
        history.push(window.Tensei.getPath(`resources/${resource.slug}`))
      })
      .catch(onErrorCatch)
  }
  ...
  const setDefaultFormData = () => {
    ...
    window.Tensei.client
      ...
      .catch(() => {
        window.Tensei.error(`${resource.name} with ID ${params.id} not found.`)
      })
  }
          ...
          >
            {mainbarFields.map(field => {
              ...
              return (
                <div
                  key={field.inputName}
                  className={`mb-5 ${sidebarVisible ? 'w-full' : 'w-3/5'}`}
                >
                  <Component
                    form={form}
                    field={field}
                ...
                return (
                  <div
                    ...
                    className={`bg-white border border-tensei-gray-600 rounded-lg p-8 ${
                      index === sidebarFields.length - 1 ? '' : 'mb-8'
                    }`}
                  >
                    <Component
                      form={form}
