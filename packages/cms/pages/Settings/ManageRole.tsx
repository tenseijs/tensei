...
const ManageUser: React.FC<ResourceProps> = ({
  ...
  useEffect(() => {
    if (editing) {
      ...
      setPermissionSlugs(editing[permissionsInputKey].map((p: any) => p.slug))
    }
  }, [editing])
  ...
  useEffect(() => {
    window.Tensei.client
      .get(
        resource.slug === 'admin-roles'
          ? 'admin-permissions'
          : window.Tensei.state.config.pluginsConfig.auth.permission.slug
      )
      .then(({ data }) => {
  ...
  const onCheckboxSelected = (permissionSlug: string, checked: boolean) => {
    if (permissionSlugs.includes(permissionSlug)) {
      setPermissionSlugs(permissionSlugs.filter(p => p !== permissionSlug))
    } else {
  ...
  return (
    <>
      ...
      <Modal
        ...
        setOpen={Boolean(editing) ? () => setEditing(null) : setCreating}
      >
        <div className="py-6">
          <div className="mb-5">
            <TextInput
            ...
            <Accordion
              ...
              onChange={toggleIndex => {
                ...
                if (accordions.includes(toggleIndex)) {
                  setAccordions(accordions.filter(a => a !== toggleIndex))
                } else {
                  setAccordions([...accordions, toggleIndex].sort())
                }
              }}
            >
                        ...
                        <Checkbox
                          ...
                          checked={
                            currentSlugs.filter(s =>
                              permissionSlugs.includes(s)
                            ).length === currentSlugs.length
                          }
                          onClick={event => event.stopPropagation()}
                          reverse
                          disabled={
                            editing ? editing.slug === 'super-admin' : undefined
                          }
                          className="flex"
                          label={r.label}
                          ...
                          onChange={event => {
                            if (event.target.checked) {
                              setPermissionSlugs([...newSlugs, ...currentSlugs])
                            } else {
                              setPermissionSlugs(newSlugs)
                            }
                          }}
                        />
