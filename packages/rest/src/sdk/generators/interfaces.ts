...
const getRelationshipFieldsForRelatedResource = (
...
const getResourcePopulateFields = (
  ...
  getRelationshipFieldsForRelatedResource(resource).forEach(field => {
    ...
    if (relatedResource) {
      const nestedPopulates = getRelationshipFieldsForRelatedResource(
        relatedResource
      ).map(
        nestedField =>
          `${field.relatedProperty.name}.${nestedField.relatedProperty.name}`
      )
  ...
  if (possiblePopulates.length === 0) {
    return `export type ${resource.data.pascalCaseName}PopulateFields = ''`
  }

  return `
	export type ${
    resource.data.pascalCaseName
  }PopulateFields = ${possiblePopulates.map(field => `'${field}'`).join('|')}
	`
}
...
const getResourceFieldsSelectList = (resource: ResourceContract) => {
  ...
  const possibleSelectFields = resource.data.fields.filter(
  ...
  return `export type ${
    resource.data.pascalCaseName
  }SelectFields = ${possibleSelectFields
...
const getFieldWhereQueryInputTypes = (
  field: FieldContract,
  resources: ResourceContract[]
) => {
  if (field.property.type === 'boolean') {
  ...
  return `
        export interface ${resource.data.pascalCaseName}WhereQueryInput {
            ${topLevelOperators.map(
              operator =>
                `${operator}?: ${
                  operator === '_not' ? interfaceName : `${interfaceName}[]`
                }`
            )},
            ${resource
              ...
              .map(
                field =>
                  `${field.databaseField}?: ${getFieldWhereQueryInputTypes(
                    field,
                    resources
                  )}`
              )}
...
const getInsertResourceInterface = (
  resource: ResourceContract,
  resources: ResourceContract[]
) => {
...
const getUpdateResourceInterface = (
  resource: ResourceContract,
  resources: ResourceContract[]
) => {
...
export const generateResourceInterfaces = async ({
  resources
}: PluginSetupConfig) => {
  return `
    ${resources
