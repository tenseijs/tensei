...
if (!targetDir) {
  ...
  console.log(
    `  ${chalk.cyan('yarn create tensei-app')} ${chalk.green('my-tensei-app')}`
  )
  process.exit(1)
}
...
const installNodeModulesTasks = ({ newAppDir }: { newAppDir: string }) => {
  return [
    {
      title: 'Running `' + 'npm' + ' install`... (This could take a while)',
      task: () => {
...
new Listr(
  ...
  .then(() => {
    ...
    ;[
      ...
      `${style.tensei(' ❖ Read the Documentation')}: https://tenseijs.com/docs`,
      '',
