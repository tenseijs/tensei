  ...
  return class SessionStore extends Parent {
    ...
    constructor(options: any = {}) {
      ...
      this.options = {
        ...
        sync: connectOptions.sync === undefined ? true : connectOptions.sync
      }
    ...
    syncDatabase = async () => {
      ...
      if (!tableExists) {
        await qb.schema.createTable(this.options.tableName, (table: any) => {
    ...
    getAll = (callback: any) => {
      return this.orm.em
        ...
        .then(sessions => sessions.map((session: any) => session.toJSON()))
    ...
    set = (sid: string | number, data: any, callback: any) => {
      ...
      return this.orm.em
        ...
        .then((session: any) => {
          ...
          return this.orm.em.persistAndFlush(newSession).then(() => newSession)
        })
        .then(value => callback(null, value))
        .catch(error => callback(error, null))
    }
    destroy = (sid: string | number, callback: any) => {
      return this.orm.em
        ...
        .then(session => {
          if (session) {
            return this.orm.em.removeAndFlush(session).then(() => session)
          }
