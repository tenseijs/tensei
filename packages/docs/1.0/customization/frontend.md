...
When building custom Nova plugins, resource plugins, cards, and fields, you may use a variety of helpers that are globally available to your JavaScript components.
...
```js
Nova.request()
  .get("/nova-vendor/stripe-inspector/endpoint")
  .then((response) => {
    // ...
  });
```
