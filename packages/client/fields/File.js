        ...
        return (
            <div className="TextField">
                <div className="flex items-center">
                    <Button onClick={() => this.inputFile.current.click()} loading={this.state.loading}>Choose file</Button>
                    <span className="ml-2 text-sm">{this.state.uploadedFile || 'No file selected'}</span>
                </div>
                <input type="file" className="hidden" ref={this.inputFile} onChange={(e) => this.uploadFile(e)} />
            </div>
        )
    }
}
