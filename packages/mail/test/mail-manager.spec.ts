...
test.group('Mail Manager', group => {
  ...
  test('extend mailer by adding a custom driver', async assert => {
    ...
    assert.deepEqual(
      manager['makeExtendedDriver']('marketing', 'mydriver').driver,
      mydriver
    )
  })
})
...
test.group('Mail Manager | Cache', group => {
  test('close driver and release it from cache', async assert => {
    ...
    assert.instanceOf(
      manager['mappingsCache'].get('marketing')!.driver,
      CustomDriver
    )
...
test.group('Mail Manager | Views', group => {
  test('make html view before sending the email', async assert => {
    ...
    manager.view!.registerTemplate('welcome', {
...
test.group('Mail Manager | preview', group => {
  test
    .skip('Mail.preview should return the preview url', async assert => {
      const config = {
        mailer: 'marketing',
        mailers: {
          marketing: {
  ...
  test
    .skip('multiple calls to preview should use one account', async assert => {
      const config = {
        mailer: 'marketing',
        mailers: {
          marketing: {
