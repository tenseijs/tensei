...
export class MailManager
  ...
  private setupViews() {
    const cacheViews = !!(
      process.env.CACHE_VIEWS || process.env.NODE_ENV === 'production'
    )
    ...
    const file = (name: string) =>
      Fs.readFileSync(
        Path.resolve(__dirname, `templates/${name}.edge`)
      ).toString('utf8')
    ...
    edge.mount(this.mailsPath || Path.resolve(process.cwd(), 'mails'))
  ...
  public trap(callback: TrapCallback) {
    const { FakeDriver } = require('./drivers/Fake')
    this.fakeMailer = new Mailer(
      'fake' as any,
      this,
      false,
      new FakeDriver(callback)
    )
  }
  ...
  public async closeAll(): Promise<void> {
    await Promise.all(
      Array.from(this['mappingsCache'].keys()).map((name: unknown) =>
        this.close(name as any)
      )
    )
  }
