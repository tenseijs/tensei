 ...
 * driver
 */
export class Mailer<Name extends keyof MailersList>
  implements MailerContract<Name> {
  ...
  private ensureView(methodName: string) {
    if (!this.manager.view) {
      throw new Error(
        `"@adonisjs/view" must be installed before using "message.${methodName}"`
      )
    }
  }
  ...
  private setEmailContent({ message, views }: CompiledMailNode) {
    if (!message.html && views.html) {
      ...
      message.html = this.manager.view!.render(
        views.html.template,
        views.html.data
      )
    }
    ...
    if (!message.text && views.text) {
      ...
      message.text = this.manager.view!.render(
        views.text.template,
        views.text.data
      )
    }
    ...
    if (!message.watch && views.watch) {
      ...
      message.watch = this.manager.view!.render(
   ...
   */
  public options(
    options: DriverOptionsType<MailersList[Name]['implementation']>
  ): this {
