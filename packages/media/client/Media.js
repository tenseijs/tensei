...
export const Media = ({ detailId, relatedResource }) => {
  ...
  const relatedField = relatedResource
    ? relatedResource.fields.find(
        f => f.name === resource.name && ['OneToMany'].includes(f.fieldName)
      )
    : undefined
  ...
  return (
    <Wrapper>
      ...
      <Heading as="h2" className="media-mb-5 media-text-2xl">
        {detailId && relatedField ? relatedField.label : 'Media Library'}
      </Heading>
      ...
      {!loading && data.data.length === 0 ? (
        <div className="media-w-full media-h-full media-flex media-flex-col media-items-center media-justify-center media-my-8">
          ...
          <p>No {resource.name.toLowerCase()} matched the given criteria.</p>
        </div>
      ) : null}
          ...
          {data.data.map(file => {
            ...
            return (
              <Card
                ...
                file={file}
                checked={checked}
                onCheckboxChange={event => onCheckboxChange(file, event)}
              />
            )
          })}
        </div>
