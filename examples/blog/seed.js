...
const postBuilder = build('Post', {
    fields: {
        ...
        category: fake(f => f.random.arrayElement(['angular', 'javascript', 'mysql', 'pg'])),
...
require('./flamingo')
    ...
    .then(async ({ databaseClient: knex }) => {
        ...
        const posts = Array(500).fill(undefined).map(() => postBuilder())
        const users = Array(500).fill(undefined).map(() => userBuilder())
        const tags = Array(500).fill(undefined).map(() => tagsBuilder())
        const comments = Array(500).fill(undefined).map(() => commentsBuilder())
        const posts_tags = Array(500).fill(undefined).map(() => postsTagsBuilder())
        const administrators = Array(50).fill(undefined).map(() => administratorBuilder())
